...
/**
 * Validates that the given data is a valid DingtalkCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): DingtalkCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<DingtalkCredentials>;
  ...
  return creds as DingtalkCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<DingtalkCredentials> | null = null;
   ...
   *
   * @param dingtalkClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    forceRefresh = false,
  ): Promise<DingtalkCredentials> {
    try {
      // Check if credentials file has been updated by other sessions
      await this.checkAndReloadIfNeeded(dingtalkClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(dingtalkClient, now);
  ...
  private async performFileCheck(
    dingtalkClient: IDingtalkOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(dingtalkClient);
  ...
  /**
   * Load credentials from the file system into memory cache and sync with dingtalkClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with dingtalkClient atomically - rollback on failure
      try {
        if (dingtalkClient) {
          dingtalkClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = dingtalkClient.getCredentials();
      ...
      await this.forceFileCheck(dingtalkClient);
      ...
      if (
        ...
        // No need to call dingtalkClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await dingtalkClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: DingtalkCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), DINGTALK_DIR, DINGTALK_LOCK_FILENAME);
  }
   ...
   */
  getCurrentCredentials(): DingtalkCredentials | null {
    return this.memoryCache.credentials;
  }
