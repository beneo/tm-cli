...
describe('Ctrl+C exit', () => {
    ...
    it.skipIf(process.platform === 'win32')('should exit gracefully on second Ctrl+C', async () => {
        ...
        await rig.poll(() => output.includes('Press Ctrl+C again to exit'), 1500, 50);
        // Send second Ctrl+C
        ptyProcess.write(String.fromCharCode(3));
    ...
    it.skipIf(process.platform === 'win32')('should exit gracefully on second Ctrl+C when calling a tool', async () => {
        const rig = new TestRig();
        await rig.setup('should exit gracefully on second Ctrl+C when calling a tool');
        const childProcessFile = 'child_process_file.txt';
        rig.createFile('wait.js', `setTimeout(() => require('fs').writeFileSync('${childProcessFile}', 'done'), 5000)`);
        const { ptyProcess, promise } = rig.runInteractive();
        let output = '';
        ptyProcess.onData((data) => {
            output += data;
        ...
        const childProcessFileExists = fs.existsSync(path.join(rig.testDir, childProcessFile));
        expect(childProcessFileExists, 'Child process file should not exist').toBe(false);
    });
});
